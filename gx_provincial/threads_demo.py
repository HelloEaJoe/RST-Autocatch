...
def get_line_center(frame):
    ...
    middle_region = frame[row_start:row_end, col_start:col_end, :]
    # opencv里读图是BGR！！！！！！！！！！！！！！！！
...
def process_frame(name):
    ...
    net = yolov5_lite(model_path, label_path, confThreshold=0.5, nmsThreshold=0.5, objThreshold=0.5)
    ...
    while True:
        ...
        if object_exists:
            ...
            while True:
                ...
                pwm1 = int(depth_pid.calculate(target_depth, depth, 1365))
                pwm2 = int(aim_door_pid.calculate(0.5, center_x, 1500))
                print(pwm2)
                print(center_x)
                ...
                if (end_time - start_time >= 2) and (object_area >= 0.4) and (abs(center_x - 0.5) <= 0.05) and (
                        abs(center_y - 0.5) <= 0.05):
                    print('RUSH!!!!!!!!!')
...
def control_line():
    ...
    while True:
        ...
        # 有物体
            ...
            # 对准物体
            if center_x >= 0.2:
                pwm2 = int(aim_door_pid.calculate(0.5, center_x, 1495))
                print(center_x, pwm2)
                line_center = get_line_center(frame)
            # 巡线
        if line_center != False:
            print("cv line")
            pwm2 = int(aim_door_pid.calculate(0.5, line_center, 1495))
            print(line_center, pwm2)
...
def control_small_ball():
    ...
    while True:
        ...
        # pwm1 = int(depth_pid.calculate(ball_depth, depth, 1495))
        pwm1 = 1500
        ...
        # 找到了
            ...
            pwm2 = int(aim_door_pid.calculate(0.5, center_x, 1500))
            aim(pwm1, pwm2, 1500, 1500)
            print('center_x' + str(center_x) + '   center_y' + str(center_y) + "   area:" + str(object_area))  # 0.046能抓

            # 如果面积达到，并且调稳了，就可以写死去抓
            if (object_area >= 0.4) and (abs(center_x - 0.5) <= 0.1) and (abs(center_y - 0.5) <= 0.1):
...
if __name__ == '__main__':
    ...
    # 进入主线
    line()
    change_mode('line to hit')
    hit()
    change_mode('hit to hit')
    line()
    change_mode('line to door')
    door(True)# 高门
    change_mode('door to line')
    line()
    change_mode('line to door')
    door(False)
    change_mode('door to line')
    ...
    change_mode('door to line')
    line()
    change_mode('line to grip')
    grip_ball()
    change_mode('grip to loose')
    loose_ball()
