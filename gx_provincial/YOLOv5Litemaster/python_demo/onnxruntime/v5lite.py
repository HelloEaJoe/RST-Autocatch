...
class yolov5_lite():
    def __init__(self, model_pb_path, label_path, confThreshold=0.5, nmsThreshold=0.5, objThreshold=0.5):
        core = Core()
        self.model = core.read_model(model_pb_path)
        ...
        # OpenVINO input shape: (N, C, H, W)
        self.input_shape = (self.input_layer.shape[2], self.input_layer.shape[3])
    ...
    def postprocess(self, frame, outs, pad_hw):
        ...
        for i in box_index:
            ...
            self.owning_classes.append(self.classes[classIds[i]])
            self.rectangle_coordinate.append([left/frameWidth, top/frameHeight, width/frameWidth, height/frameHeight])
            self.center.append([left + (width/2), top+ (height/2)])
            self.area.append(width * height)

        return frame

    def get_rectangle_coordinate(self):
        return self.rectangle_coordinate

    def get_owning_classes(self):
        return self.owning_classes
    ...
    def drawPred(self, frame, classId, conf, left, top, right, bottom): # classId指的是一个索引，代表了是第几类，每次调用drawPred画一个框
        # Draw a bounding box.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--imgpath', type=str, default='/home/wangguocun/YOLOv5-Lite-master/bus.jpg', help="image path")
    parser.add_argument('--modelpath', type=str, default='/home/wangguocun/YOLOv5-Lite-master/v5lite-e.onnx', help="onnx filepath")
