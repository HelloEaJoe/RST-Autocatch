...
class RobotStateMachine:
    ...
    def run(self):
        ...
        try:
            while self.current_state != State.STOP and (time.time()-total_timer_start_time <= 120):
    ...
    def _state_move_away(self):
        ...
        while time.time() - start_move_away_time < 3.0:
            self._select_ball()  # 刷新目标状态
            self._do_move_forward(0.1)
        print("完成: 已移开。")
        return State.FIND_BALL
    
    def _state_mv_and_dive(self,move_period=3.0):
        """11.移动并下潜状态 (阻塞)"""
        print("任务：移动并下潜至工作深度")
        set_angle(55)
        pwm_held = False
...
# 以下为状态机辅助函数实现
    ...
    def _do_spin(self,spin_period=0.1):
        """执行原地旋转"""
        pwm1 = 1410  # 保持下潜
        pwm2 = 1670  # 右转,这个数和控制转多少圈有关，尽量比一圈多一点1670
        pwm3 = 1500  # 不前进
        aim(pwm1, pwm2, pwm3)
...
if __name__ == "__main__":
    robot_sm = RobotStateMachine(work_depth=0.68)
    robot_sm.run()
